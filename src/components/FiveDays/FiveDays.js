...
const FiveDays = ({ setFive, day, setDay, weatherFive, days, weather, temp, setTemp }) => {
		...
		<section className="weather-five">
			<div className="weather-five__current">
				...
				<div className="weather__temps">
					<img src={`http://openweathermap.org/img/wn/${weather.weather[0].icon}@2x.png`} alt="Alt" />
					<p className="weather__temp weather-five__temp">{
						(temp === 'C')
							? Math.trunc(weather.main.temp - 273.15)
							: Math.trunc((weather.main.temp - 273.15) * 9 / 5 + 32)
					}</p>
				...
				<p className="weather-five__descr">
									...
									if (el.dt_txt.includes(item))
										return (
											<li key={el.dt_txt} className="weather-five__menu-list">
												...
												<p className="weather-five__menu-title">
													{el.weather[0].main}
												</p>
											</li>
										)
